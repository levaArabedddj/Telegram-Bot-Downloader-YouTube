...
async def download_with_yt_dlp(url: str, outtmpl: str, status_message):
    ...
    def ytdl_progress_hook(d):
        loop.call_soon_threadsafe(lambda: asyncio.create_task(progress_hook_async(d)))
...
async def download_audio_with_yt_dlp(url: str, outtmpl_pattern: str, status_message):
    ...
    async def progress_hook_async(d):
        try:
            ...
            if st == 'downloading':
                percent = d.get('_percent_str', '0%').strip()
                try:
                    await status_message.edit_text(f"⏳ Скачивание (аудио): {percent}")
                except BadRequest:
                    pass
            elif st == 'finished':
                try:
    ...
    try:
        await loop.run_in_executor(None, run_download)
        # после завершения найдем mp3 в той же папке
...
# Новый хендлер: /audio <link> или ответ на сообщение с ссылкой — возвращает MP3
